...
@router.get('/api/v1/posts/latest')
...
@router.get('/api/v1/posts', status_code=status.HTTP_200_OK, response_model=List[Post])
def posts(db: Session = Depends(get_db)):
    posts = services.get_posts(db)
    return posts


@router.get('/api/v1/posts/{post_id}', status_code=status.HTTP_200_OK, response_model=Post)
...
@router.post('/api/v1/posts', status_code=status.HTTP_201_CREATED, response_model=Post)
def create_post(post: CreatePost, db: Session = Depends(get_db)):
    post = services.create_post(db, post)
    return post


@router.put('/api/v1/posts/{post_id}', status_code=status.HTTP_200_OK, response_model=Post)
